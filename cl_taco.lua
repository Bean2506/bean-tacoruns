...
function TakeDeliver()
	...
	--TriggerEvent('notification', ('You started taco a run.'), 1)
	exports['mythic_notify']:SendAlert('inform', 'You started a taco run', 5000)
		...
		for k,v in pairs(Config.Tacocook) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				...
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Press E to make'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						--ESX.TriggerServerCallback('Reload_Tacoruns:getPlayers', function(plys)
							--print(plys)
							--if Config.Meat > 0 then
								TaskStartScenarioInPlace(PlayerPedId(), "PROP_HUMAN_BBQ", 0, true)
								local finished = exports['progressBars']:startUI(10000,"Making tacos")
								Citizen.Wait(10000)
								ClearPedTasksImmediately(PlayerPedId())
								TriggerServerEvent('Reload_Tacoruns:add', 'item', 1, Config.Item)
								TriggerServerEvent('Reload_Tacoruns:remove', 'item', 1, Config.Meat)
							--else
							--	exports['mythic_notify']:SendAlert('inform', 'You do not have enough ingredients', 5000)
							--end
						--end)	
					end
				end
			...
			local coords = GetEntityCoords(GetPlayerPed(-1))
				...
				if not inMission then
					...
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						...
						local finished = exports['progressBars']:startUI(9700,"Packaging tacos into bag")
						Citizen.Wait(9700)
						ClearPedTasks(PlayerPedId())
						TriggerServerEvent('Reload_Tacoruns:remove', 'item', 1, Config.Item)
						TriggerServerEvent('Reload_Tacoruns:add', 'item', 1, Config.Package)
					end
				end
		end
	end
	...
	Citizen.Wait(0)
		for k,v in pairs(Config.Tacopackage) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Press E to put bag out for delivery'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						ClearPedSecondaryTask(PlayerPedId())
						...
						local finished = exports['progressBars']:startUI(2700,"Packaging taco")
						Citizen.Wait(2700)
						ClearPedTasks(PlayerPedId())
						TriggerServerEvent('Reload_Tacoruns:remove', 'item', 1, Config.Package)
						exports['mythic_notify']:SendAlert('inform', 'You put a taco bag out for delivery', 5000)
					end
				end
		end
	end
...
--				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
--				if not inMission then
--					DrawText3D(v.x, v.y, v.z, ('Press E to deliver food'))	
--					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
--						TriggerServerEvent("Reload_Tacoruns:server")
--						ClearPedSecondaryTask(PlayerPedId())
--						loadAnimDict( "anim@heists@keycard@" ) 
--						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
--						--exports['mythic_notify']:DoHudText('delivery', 'You put a taco bag out for delivery', { ['background-color'] = '#ffffff', ['color'] = '#000000' })
--						exports['mythic_notify']:SendAlert('inform', 'You started a delivery', 5000)
--						Citizen.Wait(850)
--						ClearPedTasks(PlayerPedId())
--						TakeDeliver()
--						TriggerServerEvent('Reload_Tacoruns:add', 'item', 1, Config.Package)
			...
			local coords = GetEntityCoords(GetPlayerPed(-1))
				...
				if not inMission then
					...
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						...
						ClearPedTasks(PlayerPedId())
						inMission = false
						--TriggerEvent('notification', ('You canceled the taco run.'), 2)
						exports['mythic_notify']:SendAlert('inform', 'You cancelled the taco run', 5000)
					end
				end
		end
	end
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 20.0 then
			...
			if IsControlJustReleased(0, 38) and d < 5.0 then
				...
				TriggerServerEvent('Reload_Tacoruns:check')
				...
				exports['mythic_notify']:SendAlert('success', 'Drop off success', 5000)
				TriggerServerEvent('Reload_Tacoruns:add', 'money', reward)
				TriggerServerEvent('Reload_Tacoruns:remove', 'item', 1, Config.Package)
				TriggerServerEvent('Reload_Tacoruns:add', 'item', 1, Config.Weed)
			end
		end
	end
end)
